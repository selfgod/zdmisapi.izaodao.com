...
class Parser {
    ...
    public static function format($varName, $rule, $params) {
        ...
        return static::formatAllType($type, $value, $rule, $params);
    }
     ...
     * @throws BadRequestException
     */
    protected static function formatAllType($type, $value, $rule, $params) {
        $diKey = '_formatter' . ucfirst($type);
        $diDefault = '\\Base\\Request\\Formatter\\' . ucfirst($type) . 'Formatter';

        $formatter = Di::getInstance()->get($diKey);
        if ($formatter === null) {
            Di::getInstance()->set($diKey, $diDefault);
            $formatter = Di::getInstance()->get($diKey);
        }
        if (!($formatter instanceof Formatter)) {
            throw new BadRequestException("{$rule['name']}参数规则的类型({$type})非法");
        }

        return $formatter->parse($value, $rule, $params);
    }
}
