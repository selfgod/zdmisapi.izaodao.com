...
class FileFormatter extends BaseFormatter implements Formatter {
    ...
    public function parse($value, $rule, $params) {
        ...
        if (!isset($_FILES[$index]) || !is_array($_FILES[$index])) {
            throw new BadRequestException("缺少上传文件：{$index}");
        }
    ...
    protected function parseOne($file, $rule)
    {
        ...
        if ($file['error'] != UPLOAD_ERR_OK) {
            throw new BadRequestException("上传文件失败，error = {$file['error']}");
        }
        ...
        if (!empty($rule['ext'])) {
            ...
            if (!$ext) {
                $jext = json_encode($rule['ext']);
                throw new BadRequestException("上传失败不是文件类型 {$jext}");
            }
