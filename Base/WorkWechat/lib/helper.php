...
function http_get($url){
    ...
    $content = json_decode($sContent['content'], true);
    if($content['errcode'] == 42001 || $content['errcode'] == 40014){
        $query = parse_url($url);
        $arr_query = convertUrlQuery($query);
        $agentid = isset($arr_query['agentid']) ? $arr_query['agentid'] : 'txl';
        $path = __DIR__."/../cache/$agentid.php";
        $path = realpath($path);
...
function http_post($url,$param,$post_file=false){
    ...
    $content = json_decode($sContent['content'], true);
    if($content['errcode'] == '42001' || $content['errcode'] == '40014'){
        $agentid = isset($param['agentid'])? $param['agentid'] :'txl';
        $path = __DIR__."/../cache/$agentid.php";
        $path = realpath($path);
        $data = json_decode(get_php_file($path));
        $data->expire_time = time() - 7000;
...
//加载本地的应用配置文件
function loadConfig(){
    return json_decode(get_php_file(realpath(__DIR__."/../wx_config.php")));
}
